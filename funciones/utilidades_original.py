...
def local_op(n, site, op):
    """
    Construye el operador 'op' actuando en el sitio 'site'
    de un sistema de n qutrits.
...
def mul_monomials(m1, m2):
    return canonicalize_monomial(tuple(m1) + tuple(m2))
...
def comm_local_local(op1, op2):
    cached = _comm_local_local_cached(op1, op2)
...
def mono_to_string(mono):
    """
    Convierte un monomio a string legible.
    """
    if len(mono) == 0:
        return "1"
    return " ".join([f"J{axis}{site}" for site, axis in mono])


def print_expr(expr, max_terms=20):
    """
    Imprime una expresión de manera legible.
    """
    if not expr:
        print("0")
        return
    ...
    for mono, coeff in items[:max_terms]:
        print(f"{coeff} * {mono_to_string(mono)}")

    if len(items) > max_terms:
        print(f"... ({len(items) - max_terms} términos más)")
...
Lz = np.array([[1,0,0],[0,0,0],[0,0,-1]])
Lz = qt.Qobj(Lz)
def LZ(N, k):
    """Create a S3 operator at position k from
    a Hilbert space of N qutrits
    """
    L = [qt.qeye(3)]*N
    L[k] = Lz
    return qt.tensor(L)

#Lx = np.array([[0,-1,-1],[-1,0,-1],[-1,-1,0]])
Lx = np.array([[0,1,0],[1,0,1],[0,1,0]])
Lx = qt.Qobj(Lx)
def LX(N, k):
    """Create a S3 operator at position k from
    a Hilbert space of N qutrits
...
Ly = np.array([[0, -1j, 0],[1j,  0, -1j],[0,  1j,  0]])
Ly = qt.Qobj(Ly)
def LY(N, k):
    """Create a S3 operator at position k from
    a Hilbert space of N qutrits
...
def cost_function_fast(params, spectra, initial_state, Hf):
    psi = build_ansatz_fast(params, spectra, initial_state)
...
def index_by_grafo_id(results):
    ...
    for item in results:
        ...
        gid = int(item["grafo_id"])

        if gid in indexed:
            raise ValueError(f"grafo_id repetido: {gid}")
